...
PrayerTimeCalculator.java: Prayer Times Calculator (ver 1.0)
Copyright (C) 2007-2010 PrayerTimeCalculators.org
...
TERMS OF USE:
	...
	with a link back to PrayerTimeCalculators.org.
...
public class PrayerTimeCalculator {
    ...
    public double getTimeZoneWithDST() {
        TimeZone timez = TimeZone.getDefault();
        double hoursDiff = (timez.getRawOffset() / 1000.0) / 3600;
        if (detectDaylightSaving() > 0){
            hoursDiff += (detectDaylightSaving() / 3600000);
        }
        return hoursDiff;
    }
    ...
    public double getTimeZoneWithDSTCheck(Date date){
        if (TimeZone.getDefault().inDaylightTime(date)){
            return getTimeZoneWithDST();
        } else {
            return getBaseTimeZone();
        }
    }
    ...
    public static void main(String[] args) {
        ...
        PrayerTimeCalculator prayers = new PrayerTimeCalculator();
